...
def show_home():
    ...
    pl = profit_and_loss(conn, from_date=DATE_FROM, to_date=DATE_TO,
                         ...
                         ledger_groups=_ledger_groups_filter)
    ...
    # ── VOUCHER SUMMARY (auto-adapts to any company) ────────────────────
    st.markdown("---")
    st.subheader("Voucher Summary")
    _vch_data = voucher_summary(conn, from_date=DATE_FROM, to_date=DATE_TO,
                                voucher_types=_vch_types_filter)
    if _vch_data:
        ...
        _vch_cols = st.columns(min(len(_vch_data), 4))
        ...
        if len(_vch_data) > 4:
            with st.expander(f"All {len(_vch_data)} voucher types"):
                st.dataframe(
                    _vch_df[["Voucher Type", "Count", "Formatted Amount"]].rename(
                        columns={"Formatted Amount": "Total Amount"}
                    ),
                    use_container_width=True,
                    hide_index=True,
                )

    # ── DYNAMIC STOCK / INVENTORY SECTION ────────────────────────────────
    if _filters.get("has_stock"):
        ...
        if _stock_data:
            ...
            _total_stock_value = sum(s["closing_value"] for s in _stock_data)
            _stock_groups = list(set(s["group"] for s in _stock_data if s["group"]))
            ...
            if _filters.get("has_godowns"):
                ...
                if _godown_data:
                    st.markdown(f"**Godowns:** {', '.join(g['name'] for g in _godown_data)}")

            with st.expander("Stock Item Details"):
                _stock_df["closing_value"] = _stock_df["closing_value"].apply(fmt)
                st.dataframe(
                    _stock_df[["name", "group", "closing_value"]].rename(
                        columns={"name": "Item", "group": "Group",
