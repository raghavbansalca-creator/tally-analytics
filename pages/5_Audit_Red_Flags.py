...
safe division, safe list operations.
"""
...
def status_badge(status):
    """Return colored text for status (for expander headers)."""
    s = (status or "").lower()
    if s == "pass":
        return ":green[PASS]"
    elif s == "fail":
...
if run_all_checks is None:
    st.error("audit_engine module not found. Cannot run audit checks.")
    st.stop()
...
# Safe: results might not be a dict
if not isinstance(results, dict):
    st.error("Unexpected audit result format.")
    st.stop()
...
try:
    risk_score = float(summary.get("risk_score", 0) or 0)
except (ValueError, TypeError):
    risk_score = 0
...
try:
    ...
    st.warning(f"Could not render risk gauge: {e}")
        ...
        try:
            ...
            if key == "benfords_law":
                ...
                if dist and isinstance(dist, dict):
                    ...
                    observed_vals = []
                    for d in digits:
                        dd = dist.get(d, dist.get(str(d), {}))
                        if not isinstance(dd, dict):
                            dd = {}
                        expected_vals.append(dd.get("expected", 0) or 0)
                        observed_vals.append(dd.get("observed", 0) or 0)
            ...
            # ---- 2. Duplicate Invoices ----
                ...
                if exact and isinstance(exact, list):
                    ...
                    st.dataframe(safe_df(exact, ["voucher_number", "party", "type", "date", "duplicate_count"]),
                                 use_container_width=True, hide_index=True)
                if amt_dupes and isinstance(amt_dupes, list):
            ...
            # ---- 3. Voucher Gaps ----
                ...
                if isinstance(gaps_by_type, list):
            ...
            # ---- 4. Sunday/Holiday Entries ----
                ...
                sun_count = data.get("sunday_count", len(sun) if isinstance(sun, list) else 0)
                hol_count = data.get("holiday_count", len(hol) if isinstance(hol, list) else 0)
                st.markdown(f"**Sunday entries:** {sun_count}  |  **Holiday entries:** {hol_count}")
                if sun and isinstance(sun, list):
                    ...
                    try:
                        months = [s.get("date", "").split("-")[1] for s in sun if isinstance(s, dict) and s.get("date") and "-" in str(s.get("date", ""))]
                        if months:
            ...
            # ---- 5. Cash Limits ----
                ...
                if breaches and isinstance(breaches, list):
                    ...
                    cash_ledgers = data.get("cash_ledgers", [])
                    ledger_str = ", ".join(cash_ledgers) if isinstance(cash_ledgers, list) else str(cash_ledgers)
                    st.caption(f"Threshold: Rs 2,00,000 per Section 269ST. Cash ledgers: {ledger_str}")
