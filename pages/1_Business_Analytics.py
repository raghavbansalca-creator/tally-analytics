...
ratios = key_ratios(conn, date_from=DATE_FROM, date_to=DATE_TO)
...
eff_data = collection_efficiency(conn, date_from=DATE_FROM, date_to=DATE_TO)
...
with col1:
    ...
    customers = top_customers_by_sales(conn, 15, date_from=DATE_FROM, date_to=DATE_TO,
                                         voucher_types=_vch_types_filter)
    if customers:
...
with col2:
    ...
    suppliers = top_suppliers_by_purchase(conn, 15, date_from=DATE_FROM, date_to=DATE_TO,
                                            voucher_types=_vch_types_filter)
    if suppliers:
...
projections = project_cash_flow(conn, months_ahead=3, date_from=DATE_FROM, date_to=DATE_TO)
...
if projections:
    ...
    current_bal = sum(abs(b[3]) for b in bank_balances(conn, date_from=DATE_FROM, date_to=DATE_TO))
    for p in projections:
        bal_chart.append({
            "month": ml(p["month"]),
...
st.markdown("---")
try:
    _co = conn.execute("SELECT value FROM _metadata WHERE key='company_name'").fetchone()
