...
def monthly_bank_movement(conn):
    ...
    return conn.execute("""
        ...
        JOIN trn_accounting a ON a.VOUCHER_GUID = v.GUID
        JOIN mst_ledger l ON l.NAME = a.LEDGERNAME
        WHERE l.PARENT IN ('Bank Accounts', 'Bank OD A/c')
        GROUP BY month, a.LEDGERNAME
        ORDER BY month
    """).fetchall()
...
def cash_flow_statement(conn):
    ...
    """
    months_data = monthly_gross_profit(conn)
...
def project_cash_flow(conn, months_ahead=3):
...
def working_capital_analysis(conn):
...
def key_ratios(conn):
    ...
    # Months of data
    months_count = len(set(r[0] for r in monthly_sales(conn)))
...
def collection_efficiency(conn):
    """Monthly collection efficiency: receipts as % of opening debtors + sales."""
    sales_data = {r[0]: r[2] for r in monthly_sales(conn)}
    receipt_data = {}
    rows = conn.execute("""
        SELECT SUBSTR(v.DATE,1,6) as month,
               SUM(ABS(CAST(a.AMOUNT AS REAL))) as amt
